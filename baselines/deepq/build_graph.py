...
`       See the top of the file for details.
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        transition_mat_ph = tf.placeholder(tf.float32, [num_task_states, num_task_states], name="trans_mat_train")
...
def build_act_ltl_test_sample(make_obs_ph, q_func, num_actions, num_task_states, scope="deepq", reuse=None):
    """Creates the act function by using a sample of the x states the thing is currently in:
    Parameters
    ----------
    make_obs_ph: str -> tf.placeholder or TfInput
        a function that take a name and creates a placeholder of input with that name
...
`       See the top of the file for details.
    ...
    if param_noise:
        ...
        if action_sel == "argmax":
            act_f = build_act_ltl(make_obs_ph, q_func, num_actions, num_task_states, scope=scope, reuse=reuse)
        elif action_sel == "sample":
            act_f = build_act_ltl_sample(make_obs_ph, q_func, num_actions, num_task_states, scope=scope, reuse=reuse)
        elif action_sel == "expectation":
            act_f = build_act_ltl_expectation(make_obs_ph, q_func, num_actions, num_task_states, scope=scope, reuse=reuse)
        else:
            raise ValueError("action-sel arg must be either 'argmax', 'sample', or 'expectation'")
